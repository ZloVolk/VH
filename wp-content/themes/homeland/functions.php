	...
	*/
	...
	if ( ! function_exists( 'homeland_script_styles_reg' ) ) :
		function homeland_script_styles_reg () {
			...
			// Main Stylesheets
			wp_enqueue_style( 'homeland_style', get_stylesheet_uri() );
			wp_enqueue_style( 'homeland_font_awesome', $homeland_directory_uri . '/includes/font-awesome/css/font-awesome.min.css', array(), '4.5.0' );
			wp_enqueue_style( 'homeland_flexslider_style', $homeland_js_directory_uri . '/flexslider/flexslider.css' );
			wp_enqueue_style( 'homeland_magnific_style', $homeland_js_directory_uri . '/magnific-popup/magnific-popup.css' );
			wp_enqueue_style( 'homeland_select2_style', $homeland_js_directory_uri . '/select2/select2.css' );
			
			// jQuery Scripts
			wp_enqueue_script( 'jquery-masonry' );
			wp_enqueue_script( 'homeland_superfish', $homeland_js_directory_uri . '/superfish.min.js', array(), '', true );
			wp_enqueue_script( 'homeland_easing', $homeland_js_directory_uri . '/jquery.easing-1.3.min.js', array(), '', true );
			wp_enqueue_script( 'homeland_retina', $homeland_js_directory_uri . '/retina.min.js', array(), '', true );
			wp_enqueue_script( 'homeland_flexslider', $homeland_js_directory_uri . '/flexslider/jquery.flexslider-min.js', array(), '', true );
			wp_enqueue_script( 'homeland_hover_modernizr', $homeland_js_directory_uri . '/modernizr.custom.js', array(), '', true );
			wp_enqueue_script( 'homeland_elastic', $homeland_js_directory_uri . '/jquery.elastislide.min.js', array(), '', true );
			wp_enqueue_script( 'homeland_magnific', $homeland_js_directory_uri . '/magnific-popup/jquery.magnific-popup.min.js', array(), '', true );
			wp_enqueue_script( 'homeland_select2', $homeland_js_directory_uri . '/select2/select2.min.js', array(), '', true );
			wp_enqueue_script( 'homeland_validation', $homeland_js_directory_uri . '/jquery.validate.min.js', array(), '', true );

			wp_enqueue_script( 'homeland_html5', '//html5shim.googlecode.com/svn/trunk/html5.js' );
			wp_script_add_data( 'homeland_html5', 'conditional', 'lt IE 9' );
			
			// Google Maps
			wp_register_script( 'homeland_gmap_sensor', 'http://maps.google.com/maps/api/js?sensor=true' );
			wp_register_script( 'homeland_gmap_marker_clusterer', 'https://rawgit.com/googlemaps/js-marker-clusterer/gh-pages/src/markerclusterer.js' );
			wp_register_script( 'homeland_gmap', $homeland_js_directory_uri . '/gmaps.min.js' );
			wp_localize_script( 'homeland_gmap', 'wpGlobals', array( 'mapOptions' => file_get_contents( dirname(__FILE__) . '/map_style.json' )) );

			// Homepage Templates
			if(is_page_template('template-homepage-gmap.php') || is_page_template('template-homepage-gmap-large.php')) : 
				wp_enqueue_script( 'homeland_gmap_sensor' );
				wp_enqueue_script( 'homeland_gmap_marker_clusterer' );
			...
			if($homeland_bg_type == "Image" && ($homeland_theme_layout == "Boxed" || $homeland_theme_layout == "Boxed Left")) :
				wp_enqueue_script( 'homeland_backstretch', $homeland_js_directory_uri . '/jquery.backstretch.min.js', array(), '', true );
			endif;
			...
			if(is_page_template('template-blog.php') || is_page_template('template-blog-3cols.php') || is_page_template('template-blog-4cols.php') || is_page_template('template-blog-fullwidth.php') || is_page_template('template-blog-grid-left-sidebar.php') || is_page_template('template-blog-grid.php') || is_page_template('template-blog-left-sidebar.php') || is_page_template('template-blog-timeline.php') || is_page_template('template-blog-2cols.php') || is_page_template('template-blog-list-alternate.php') || is_single() || is_archive()) :
				wp_enqueue_style( 'homeland_videojs_css', $homeland_js_directory_uri . '/video/video-js.css' );
				wp_enqueue_script( 'homeland_videojs', $homeland_js_directory_uri . '/video/video.js', array(), '', true );
				wp_enqueue_script( 'homeland_tipsy', $homeland_js_directory_uri . '/tipsy/jquery.tipsy.min.js', array(), '', true );
			endif;
			...
			if(is_page_template('template-coming-soon.php')) :
				wp_enqueue_script( 'homeland_countdown_plugin', $homeland_js_directory_uri . '/countdown/jquery.plugin.min.js', array(), '', true );
				wp_enqueue_script( 'homeland_countdown', $homeland_js_directory_uri . '/countdown/jquery.countdown.min.js', array(), '', true );
			endif;

			// RTL Enable
			if ( is_rtl() ) :
				wp_enqueue_style( 'homeland_rtl_main', $homeland_directory_uri . '/css/rtl-main.css' );	
			endif;

			// Responsive Styles
			if(empty($homeland_site_layout)) :
				wp_enqueue_style( 'homeland_responsive', get_stylesheet_directory_uri() . '/responsive.css' );	
				
				if ( is_rtl() ) :
					wp_enqueue_style( 'homeland_rtl_responsive', $homeland_directory_uri . '/css/rtl-responsive.css' );	
				endif;
			
				if($homeland_theme_mobile_menu == "Modern") :	
					wp_enqueue_script( 'homeland_slick_nav_js', $homeland_js_directory_uri . '/jquery.slicknav.min.js', array(), '', true );
				else :
					wp_enqueue_script( 'homeland_mobile_menu', $homeland_js_directory_uri . '/jquery.mobilemenu.min.js', array(), '', true );
				endif;
			endif;

			// Custom jQueries
			wp_enqueue_script( 'homeland_custom_js', $homeland_js_directory_uri . '/custom.js', array(), '', true );
		}
	endif;
	...
	if ( ! function_exists( 'homeland_theme_setup' ) ) :
		function homeland_theme_setup() {
			...
			register_nav_menus( array(
				'primary-menu' => __( 'Primary Menu', 'homeland' ),
				'footer-menu' => __( 'Footer Menu', 'homeland' )
			) );
	...
	if ( ! function_exists( 'homeland_custom_admin_style' ) ) :	
		function homeland_custom_admin_style() {
			...
			if(is_rtl()) :
				wp_enqueue_style( 'homeland_rtl_style_admin', $homeland_directory_uri . '/rtl.css' );
			endif;
		}
	endif;
				    	...
				    	<?php if(is_tax( 'homeland_property_location' ) || is_tax( 'homeland_property_type' ) || is_tax( 'homeland_property_status' ) || is_tax( 'homeland_property_amenities' )) : ?>
			    			map = new GMaps({
						      ...
						      lat: <?php echo $homeland_lat_main; ?>,
									...
									zoom: <?php echo $homeland_home_map_zoom_value; ?>,
									styles : snazzyMap,
					    	...
					    	map.setCenter(<?php echo $homeland_lat_main; ?>, <?php echo $homeland_lng_main; ?>); 
							<?php else : ?>
			    			map = new GMaps({
						      div: '#map-homepage',
						      scrollwheel: false,
						   	...
						   	$(document).ready(function(){
					        	...
					        	scrollwheel: false,
										...
										<?php	else : ?>
											lat: <?php echo $homeland_lat; ?>, lng: <?php echo $homeland_lng; ?>,
										<?php endif; ?>
											zoom: <?php echo $homeland_property_map_zoom_value ?>,
											styles : snazzyMap,
						      	});
	...
	if ( ! function_exists( 'homeland_google_map' ) ) :
		...
		$homeland_marker_tip = get_option('homeland_map_marker');
		$homeland_marker_window = get_option('homeland_map_window');		
			...
			(function($) {
			   ...
			   $(document).ready(function(){
		      	...
		      	map.addMarker({
			        	lat: <?php echo $homeland_lat; ?>,
						lng: <?php echo $homeland_lng; ?>,
			        	title: "<?php echo stripslashes($homeland_marker_tip); ?>",
			        	infoWindow: {
				    		content: "<p><?php echo stripslashes($homeland_marker_window); ?></p>"
				    	}
		      	});
			   });
			})(jQuery);					
	...
	if ( ! function_exists( 'homeland_change_default_site_title' ) ) :
		function homeland_change_default_site_title( $homeland_title ){
			...
			if('homeland_properties' == $homeland_screen->post_type) :
				$homeland_title = __( 'Enter property name', 'homeland' );
			elseif('homeland_portfolio' == $homeland_screen->post_type) :
				$homeland_title = __( 'Enter portfolio name', 'homeland' );
			elseif('homeland_services' == $homeland_screen->post_type) :
				$homeland_title = __( 'Enter services name', 'homeland' );
			elseif('homeland_testimonial' == $homeland_screen->post_type) :
				$homeland_title = __( 'Enter name', 'homeland' );
			elseif('homeland_partners' == $homeland_screen->post_type) :
				$homeland_title = __( 'Enter partner name', 'homeland' );
			endif;
				...
				<div id="dropdown" class="theme-menu clear">
					<ul id="main-menu" class="sf-menu">
						<li <?php sanitize_html_class( post_class( $homeland_class ) ); ?>>
							<a href="<?php echo esc_url( home_url() ); ?>"><?php _e( 'Home', 'homeland' ); ?></a>
						</li>
			...
			?>
				<div class="footer-menu">
					<ul id="menu-footer-menu" class="clear">
						<li><a href="<?php echo esc_url( home_url() ); ?>"><?php _e( 'Home', 'homeland' ); ?></a></li>
						<?php wp_list_pages( 'title_li=&sort_column=menu_order' ); ?>
					</ul>
				</div>
			<?php
	...
	if ( ! function_exists( 'homeland_get_page_title' ) ) :
		function homeland_get_page_title() {
			...
			$homeland_property_archive_header_label = !empty($homeland_property_archive_header) ? $homeland_property_archive_header : __('Our Properties', 'homeland');
			$homeland_property_archive_subtitle_label = !empty($homeland_property_archive_subtitle) ? $homeland_property_archive_subtitle : __('This is your property subtitle here', 'homeland');
			$homeland_portfolio_archive_header_label = !empty($homeland_portfolio_archive_header) ? $homeland_portfolio_archive_header : __('Our Portfolio', 'homeland');
			$homeland_portfolio_archive_subtitle_label = !empty($homeland_portfolio_archive_subtitle) ? $homeland_portfolio_archive_subtitle : __('This is your portfolio subtitle here', 'homeland');
			$homeland_services_archive_header_label = !empty($homeland_services_archive_header) ? $homeland_services_archive_header : __('Our Services', 'homeland');
			$homeland_services_archive_subtitle_label = !empty($homeland_services_archive_subtitle) ? $homeland_services_archive_subtitle : __('This is your services subtitle here', 'homeland');
			$homeland_search_header_label = !empty($homeland_search_header) ? $homeland_search_header : __('Search Results', 'homeland');
			$homeland_search_subtitle_label = !empty($homeland_search_subtitle) ? $homeland_search_subtitle : __('This is your search subtitle description', 'homeland');
			$homeland_not_found_header_label = !empty($homeland_not_found_header) ? $homeland_not_found_header : __('404 Page', 'homeland');
			$homeland_not_found_subtitle_label = !empty($homeland_not_found_subtitle) ? $homeland_not_found_subtitle : __('This is your 404 not found subtitle here', 'homeland');
			$homeland_agent_profile_header_label = !empty($homeland_agent_profile_header) ? $homeland_agent_profile_header : __('Agent Profile', 'homeland');
			$homeland_agent_profile_subtitle_label = !empty($homeland_agent_profile_subtitle) ? $homeland_agent_profile_subtitle : __('This is your agent subtitle here', 'homeland');
			...
			elseif(is_archive()) : 
				echo '<h2 class="ptitle">';
					...
					elseif(is_tag()) : printf( _e('Posts Tagged: ', 'homeland'), single_tag_title() ); 
					elseif(is_day()) : printf( __( 'Daily Archives: %s', 'homeland'), get_the_date()); 
					elseif(is_month()) : printf( __( 'Monthly Archives: %s', 'homeland' ), get_the_date( _x( 'F Y', 'monthly archives date format', 'homeland' ) ) ); 
					elseif ( is_year() ) : printf( __( 'Yearly Archives: %s', 'homeland' ), get_the_date( _x( 'Y', 'yearly archives date format', 'homeland' ) ) ); 
			...
			elseif (is_singular('homeland_properties')) : 
				...
				if(!empty($homeland_address)) :
					echo '<h4 class="subtitle"><label><i class="fa fa-map-marker"></i>' . $homeland_address . '</label></h4>';
				endif;
		...
		function homeland_get_property_category() {
			...
			<div class="cat-toogles">
				<ul class="cat-list clear">
					<li class="<?php if(is_page_template('template-properties.php') || is_page_template('template-properties-2cols.php') || is_page_template('template-properties-3cols.php') || is_page_template('template-properties-4cols.php')) : echo 'current-cat'; endif; ?>"><a href="<?php echo $homeland_properties_page_url; ?>"><?php _e('All', 'homeland'); ?></a></li>
					<?php
				...
				<div class="parent clear" id="comment-<?php comment_ID(); ?>">					
					...
					<div class="comment-details">
						...
						<?php 
							...
							if ($comment->comment_approved == '0') : ?><em><?php _e('Your comment is awaiting moderation.', 'homeland'); ?></em><?php 
							endif; 					
						?>
					</div>
				</div><?php
	...
	if(!function_exists( 'homeland_pagination')) :
		function homeland_pagination() {  
			...
			echo paginate_links( array(
				...
				'format' => '?paged=%#%',
				'prev_text' => __('&laquo;', 'homeland'),
	    		'next_text' => __('&raquo;', 'homeland'),
		...
		function homeland_next_previous() { ?>
			<div class="pagination">
				<?php
				   ...
				   if($paged > 1) : ?>
				   	<div class="alignleft"><a href="<?php previous_posts(); ?>">&larr; <?php _e('Previous', 'homeland'); ?></a></div><?php
				   endif;
				    
				   if($max_num_pages_count == 1) :	    		
			    	elseif($paged < $max_num_pages_count) : ?>
			    		<div class="alignright"><a href="<?php next_posts(); ?>"><?php _e('Next', 'homeland'); ?> &rarr;</a></div><?php
			    	endif;
				?>
			</div><?php
		}
	...
	function homeland_users_properties_column($homeland_cols) {
	  	$homeland_cols['homeland_user_properties'] = __( 'Listings', 'homeland' );   
  ...
  add_filter('homeland_advance_search_parameters', 'homeland_advance_property_search');
		...
		function homeland_property_sort_order() {
			...
			<div class="clear">
                ...
                <input type="hidden" name="max-price" value="<?php echo $_GET['max-price']; ?>"><?php
				    ...
				    <label for="input_sort"><?php _e('Sort By', 'homeland'); ?></label>
							...
							<?php
								...
								if($homeland_filter_default == "Name") :
									$homeland_array = array( 
										...
										'homeland_price' => __('Price', 'homeland'), 
	...
	if(!function_exists('homeland_add_new_contact_info')) :	
		function homeland_add_new_contact_info( $homeland_contact_methods ) {
			$homeland_contact_methods['homeland_designation'] = __('Designation', 'homeland');
			$homeland_contact_methods['homeland_twitter'] = __('Twitter', 'homeland');
			$homeland_contact_methods['homeland_facebook'] = __('Facebook', 'homeland');
			$homeland_contact_methods['homeland_gplus'] = __('Google Plus', 'homeland');
			$homeland_contact_methods['homeland_linkedin'] = __('LinkedIn', 'homeland');
			$homeland_contact_methods['homeland_telno'] = __('Telephone', 'homeland');
			$homeland_contact_methods['homeland_mobile'] = __('Mobile', 'homeland');
			$homeland_contact_methods['homeland_fax'] = __('Fax', 'homeland');
	...
	if(!function_exists('homeland_create_category_fields')) :
	    ...
	    $homeland_cat_meta = get_option("category_$homeland_extra_id"); ?>
			<div class="form-field">
				<label for="homeland_cat_meta[homeland_subtitle]"><?php _e('Subtitle', 'homeland'); ?></label>
			    ...
			    <p><?php _e('Add your subtitle text here', 'homeland'); ?></p>
			</div><?php
		}
	endif;
	...
	if(!function_exists('homeland_edit_category_fields')) :
	    ...
	    $homeland_cat_meta = get_option("category_$homeland_extra_id"); ?>
			<tr class="form-field">
				<th valign="top" scope="row"><label for="homeland_cat_meta[homeland_subtitle]"><?php _e('Subtitle', 'homeland'); ?></label></th>
				<td>
					...
					<span class="description"><?php _e('Edit your subtitle text here', 'homeland'); ?></span>
				</td>		        
			</tr><?php
		}
	endif;
			...
			if($homeland_bg_type == "Image" && ($homeland_theme_layout == "Boxed" || $homeland_theme_layout == "Boxed Left")) :
				...
				elseif(is_attachment()) :
					if(!empty($homeland_attachment_bgimage)) : ?>
						<script type="text/javascript"> 
							jQuery(window).load(function() { jQuery.backstretch("<?php echo $homeland_attachment_bgimage; ?>"); }); 
						</script><?php					
					else : homeland_default_img_bg(); endif;
		...
		function homeland_header_image() {
			...
			if(is_page_template('template-homepage.php') || is_page_template('template-homepage2.php') || is_page_template('template-homepage3.php') || is_page_template('template-homepage4.php') || is_page_template('template-homepage-video.php') || is_page_template('template-homepage-revslider.php') || is_page_template('template-homepage-gmap.php') || is_page_template('template-homepage-gmap-large.php') || is_page_template('template-page-builder.php')) :  
				...
				elseif(is_search()) : 
					...
					if(!empty($homeland_header_overlay)) : echo "<div class='overlay'>&nbsp;</div>"; endif;
				elseif(is_attachment()) : 
					echo "<section class='" . $homeland_title_block_attachment . " header-bg'>";
					if(!empty($homeland_header_overlay)) : echo "<div class='overlay'>&nbsp;</div>"; endif;
				elseif(is_404()) : 
  ...
  /* Add Lightbox for gallery shortcode and image post */
	...
	if(!function_exists('homeland_metabox_agent')) :
		function homeland_metabox_agent() {
			...
			add_meta_box('homeland_authordiv', __('Agent', 'homeland'), 'post_author_meta_box', 'homeland_properties', 'normal', 'core');
		}
	endif;
	...
	if(!function_exists('homeland_custom_avatar')) :
		function homeland_custom_avatar($user) { 
			?>
				<h3><?php _e('Custom Avatar', 'homeland'); ?></h3> 
				<table class="form-table">
					<tr>
						<th>
							<label for="homeland_custom_avatar"><?php _e('Avatar', 'homeland'); ?></label>
						</th>
						<td>
							<input type="text" name="homeland_custom_avatar" id="homeland_custom_avatar" value="<?php echo esc_attr( get_the_author_meta( 'homeland_custom_avatar', $user->ID ) ); ?>" class="regular-text" /><input id="upload_image_button_avatar" type="button" value="<?php _e( 'Upload', 'homeland' ); ?>" class="button-secondary" /><br />
							<span class="description">
								<?php _e('This will override your default Gravatar or show up if you dont have a Gravatar', 'homeland'); ?><br /><strong><?php _e('Image should be 240x240 pixels', 'homeland'); ?>.</strong>
							</span>
						</td>
					</tr>
				</table>
			...
			<div class="cat-toogles">
				<ul class="cat-list clear">
					<li class="<?php echo $homeland_portfolio_current_class; ?>">
						<a href="<?php echo $homeland_portfolio_page_url; ?>"><?php _e('All', 'homeland'); ?></a>
					</li>
					<?php
						$args = array( 
