...
<article id="post-<?php the_ID(); ?>" <?php sanitize_html_class( post_class('blist clear') ); ?>>
	...
	<div class="blog-timeline-content">
		...
		<label>
			<?php the_time(get_option('date_format')); ?> / <?php the_author_meta( 'display_name' ); ?> / <?php the_category(', ') ?> / <a href="<?php the_permalink(); ?>#comments"><?php comments_number( __( 'No Comments', 'homeland' ), __( 'One Comment', 'homeland' ), __( '% Comments', 'homeland' ) ); ?></a>
		</label>
		<?php the_excerpt(); ?>
		<a href="<?php the_permalink(); ?>" class="continue">
			<?php
				if(!empty( $homeland_blog_button )) : echo $homeland_blog_button;
				else : _e( 'Learn More', 'homeland' );
				endif;
			?> &rarr;
		</a>	
