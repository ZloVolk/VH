<?php
	...
	class homeland_Widget_Video extends WP_Widget {
		...
		function homeland_Widget_Video() {		
			$widget_ops = array('classname' => 'homeland_widget-video', 'description' => __('Custom widget for video', 'homeland'));	
			parent::__construct('Video', __('Homeland: Video', 'homeland'), $widget_ops);		
		}
		...
		function widget($args, $instance) {		
			...
			if (empty($title)) $title = false;
				...
				?>
					...
					<div>			
						<div>
							<p><label for="<?php echo $this -> get_field_id($homeland_video_url); ?>"><?php _e('Video iFrame', 'homeland'); ?></label>
							...
							<small><i><?php _e( 'Please enter the entire embedd codes from vimeo, youtube or dailymotion', 'homeland' ); ?></i></small>
						</div>			
					</div>
		<?php
				}			
