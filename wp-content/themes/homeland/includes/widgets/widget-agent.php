<?php
	...
	class homeland_Widget_Agents extends WP_Widget {
		...
		function homeland_Widget_Agents() {		
			$widget_ops = array('classname' => 'homeland_widget-agents', 'description' => __('Custom widget for agents', 'homeland'));	
			parent::__construct('Agents', __('Homeland: Agents', 'homeland'), $widget_ops);		
		}
			...
			if (empty($title)) $title = false;					
					   ...
					   if (!empty( $homeland_agents->results)) :
							foreach ($homeland_agents->results as $homeland_user) :
						    	...
						    	<li class="clear">
						    		...
						    		<label>
						    			<i class="fa fa-home fa-lg"></i> <?php esc_attr( _e( 'Listed:', 'homeland' ) ); ?>
				...
				?>
					...
					<div>			
						...
						<p><small><i><?php _e( 'Top Agents List will display automatically base on the number of properties they listed...', 'homeland' ); ?></i></small></p>		
					</div>
		<?php
				}			
		}
