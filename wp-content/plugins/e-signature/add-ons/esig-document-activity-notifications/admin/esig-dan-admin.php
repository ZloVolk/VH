...
if (!class_exists('ESIG_DAN_Admin')) :
    ...
    class ESIG_DAN_Admin {
        ...
        public static function sad_document_after_send($args){
            
            $old_doc_id = $args['old_doc_id'] ; 
            $stop_email = self::get_stop_email_after_sign_meta($old_doc_id);
            if($stop_email){
                $document_id = $args['document']->document_id; 
                self::save_stop_email_after_sign_meta($document_id, $stop_email);
            }
        }
        ...
        public function enqueue_admin_scripts() {
            ...
            if (in_array($screen->id, $admin_screens)) {
                ...
                wp_enqueue_script($this->plugin_slug . '-admin-script', plugins_url('assets/js/esig-dan.js', __FILE__), array('jquery'),  esig_plugin_name_get_version(), true);
            }
        }
