...
<div class='signatures row'>
	...
	";
                ...
                if (!empty($allinvitaions)) {
                    $small_img = ESIGN_DIRECTORY_URI . "assets/images/sign-here_blank_small.jpg";
                    ...
                    foreach ($allinvitaions as $invite) {
                        ...
                        if (!$this->signature->userHasSignedDocument($invite->user_id, $doc_id)) {
                            ...
                            $sign_bottom_text .= "<br>Signed On: " . $this->document->esig_date_format($date);
                        }
                        ...
                        if ($signature_type == "typed") {
                            ...
                            $font = $api->setting->get_generic('esig-signature-type-font' . $invite->user_id);

                            $font_size = 50 - strlen($sign_data);

                            $sign = '<div class="sign-text-pdf" >
						<p style="height:5px">&nbsp;</p>
						<span class="esig-signature-type-font" style="font-family:' . $font . ';font-size:' . $font_size . '!important;" width="400" height="100px">' . $sign_data . '</span></div>';
					...
					<div class="signature-top" style="width:350px;text-align:left;background:transparent url(' . $small_img . ') left bottom no-repeat;height:100px;">
                ...
                if ($document->add_signature) {
                    ...
                    if ($signature_type == "typed") {
                        ...
                        $sign_admin = '<img src="' . plugins_url() . '/esig-save-as-pdf/includes/sigtoimage.php?uid=' . $owner->user_id . '&doc_id=old-aams&owner_id=1" width="255px" height="100px">';
                    }



                    $html .='<div class="signature-left" align="left">
					<div class="signature-top" style="width:350px;text-align:left;background:transparent url(' . $small_img . ') left bottom no-repeat;height:100px;vertical-align:bottom;">
					
					' . $sign_admin . '
					
					</div><div class="signature-top">';
                    $html .= sprintf(__("Signed By %s %s", "esig-pdf"), $owner->first_name, $owner->last_name);
                    $html .= "<br>Signed On: " . $this->document->esig_date_format($document->last_modified) . "</div></div>";
                }
		...
		<div class='footer-right'>
			...
			<br/><img src='" . ESIGN_ASSETS_DIR_URI . "/images/lock.png' width='8' height='12' alt='Audit Lock'/> " . __("Audit Trail Serial#", "esig") . " {$api->shortcode->auditReport($doc_id, $document, true)}
		</div>
	</div>";
        ...
        public function enqueue_admin_styles() {
            ...
            if (in_array($screen->id, $admin_screens)) {
                wp_enqueue_style($this->plugin_slug . '-admin-styles', plugins_url('assets/css/chosen.min.css', __FILE__), array(), esig_plugin_name_get_version());
            }
        }
        ...
        public function enqueue_admin_scripts() {
            ...
            if (in_array($screen->id, $admin_screens)) {
                ...
                wp_enqueue_script($this->plugin_slug . '-admin-script', plugins_url('assets/js/chosen.jquery.js', __FILE__), array('jquery', 'jquery-ui-dialog'), esig_plugin_name_get_version(), true);
                wp_enqueue_script($this->plugin_slug . '-admin-script1', plugins_url('assets/js/prism.js', __FILE__), array('jquery', 'jquery-ui-dialog'), esig_plugin_name_get_version(), true);
                wp_enqueue_script($this->plugin_slug . '-admin-script2', plugins_url('assets/js/main.js', __FILE__), array('jquery', 'jquery-ui-dialog'), esig_plugin_name_get_version(), true);
            }
        }
