...
class WP_E_Document extends WP_E_Model {
    ...
    public function esig_do_shortcode($document_id) {
        ...
        add_option('esig_global_document_id', $document_id);
        if ($document->document_type == 'esig_template' && class_exists('ESIG_AT_Admin')) {

            $esig_template = new ESIG_AT_Admin();
            $document_content = do_shortcode($esig_template->esig_template_document_content_insert($document_id));
        } else {
    ...
    public function copy($doc_id) {
        ...
        $event_text = sprintf(__("%s Uploaded by %s - %s IP %s", 'esig'), $doc['document_title'], $admin_name, $admin_email, esig_get_ip());

        $this->record_generic_Event($doc_id, "Upload", $event_text, $doc['date_created']);
...
EOL;
    ...
    public function recordEvent($id, $msg = null, $event_data = null, $date = null) {
        ...
        $this->wpdb->query(
                $this->wpdb->prepare(
                        "INSERT INTO " . $this->eventsTable . " (id, document_id, event, event_data, date) VALUES (null, %d,%s,%s,%s)", $id, $msg, $event_data, $date
                )
        );
        $event_id = $this->wpdb->insert_id;
     ...
     * @return Int event id
     */
    public function record_generic_Event($id, $msg = null, $event_data = null, $date = null) {
