...
class Homeland_Testimonials_Block extends AQ_Block {
	...
	function block($instance) {
		...
		if(!empty($homeland_background)) :
			?><style>.testimonial-pb-block { background: url('<?php echo $homeland_background; ?>') no-repeat center !important; background-size: cover !important; }</style><?php
		endif;

		if(!empty($homeland_overlay)) :
			?><style>.testimonial-pb-block .overlay { background-color: <?php echo $homeland_overlay; ?> !important; }</style><?php
		endif;
		...
		if ($wp_query->have_posts()) : ?>
			<!-- Testimonials -->
			<section class="testimonial-pb-block">
				<?php
					if(!empty($homeland_overlay)) :
