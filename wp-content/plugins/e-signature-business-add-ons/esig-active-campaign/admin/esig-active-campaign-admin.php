...
if (!class_exists('ESIG_ACTIVE_CAMPAIGN_Admin')) :
    ...
    class ESIG_ACTIVE_CAMPAIGN_Admin {
        ...
        private static function get_default_sad_document_id() {
            if (isset($_COOKIE[self::COOKIE_CAMPAIGN_DOC_ID])) {
                return $_COOKIE[self::COOKIE_CAMPAIGN_DOC_ID];
            }
            return false;
        }
        ...
        public function sad_document_after_save($args) {
            ...
            $campaign_list = self::get_campaign_list($old_doc_id);
            if ($campaign_list) {
                $campaign_tag = self::get_campaign_tag($old_doc_id);
                self::set_campaign_list($doc_id, json_decode($campaign_list));
                self::set_campaign_tag($doc_id, $campaign_tag);
            }
        }
...
function esigactivecampaign() {
    ...
    ESIG_ACTIVE_CAMPAIGN_Admin::set_campaign_tag($document_max_id, $_POST['esig_active_campaign_tag']);
...
function esigactivecampaigntagdelete() {
    ...
    $api = new WP_E_Api();
    if (!$api->meta->get($esigdocid, 'esig-active-campaign-tag-')) {
        echo "This tag not exists";
    } else {
        $api->meta->delete($esigdocid, 'esig-active-campaign-tag-');
    }
