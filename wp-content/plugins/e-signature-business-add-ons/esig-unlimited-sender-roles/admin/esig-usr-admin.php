    ...
    class ESIG_USR_Admin extends Esig_Roles {
        ...
        public function esig_document_after_save_data($args) {
            ...
            if ($document_status != 'esig_template') {
                $doc_id = $api->document->document_max();
            }
            ...
            if (count($_POST) > 0) {
                ...
                // saving roles option 
                $esig_settings->set_generic('esig_unlimited_roles_' . $doc_id, json_encode($esig_roles_option));
                // saving user roles option 
                $esig_settings->set_generic('esig_unlimited_users_' . $doc_id, json_encode($esig_roles_user_option));
        ...
        public function esign_document_permission($docs) {
            ...
            foreach ($docs as $doc) {
                ...
                if ($document_id) {
                    ...
                    $esig_unlimited_roles_option = json_decode($esig_settings->get_generic('esig_unlimited_roles_' . $document_id),true);
...
';
        ...
        public function document_template_allow($document_id) {
            ...
            $esign_unlimited_roles = json_decode($esig_settings->get_generic('esig_unlimited_roles_' . $document_id),true); // getting users from settings table 


  
            $esign_unlimited_users = json_decode($esig_settings->get_generic('esig_unlimited_users_' . $document_id),true); // getting users from settings table 
