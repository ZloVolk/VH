...
class ESIG_AAMS_Admin {
	...
	public function enqueue_admin_scripts() {
		...
		if(in_array($screen->id, $admin_screens))
		{
			...
			wp_enqueue_script( $this->plugin_slug . '-admin-script', plugins_url( 'assets/js/esig-auto-add-signature.js', __FILE__), array('jquery', 'jquery-ui-dialog'), ESIG_AAMS::VERSION ,true);	
			// localization 
		   ...
		   // $signature_type=$api->signature->getSignature_type_signature_id($signature_id) ;
			...
			if($document->add_signature){
				$owner_data = array(
					...
					'sign_date' => "Signed On: " . $api->document->esig_date_format($document->last_modified),
				);
