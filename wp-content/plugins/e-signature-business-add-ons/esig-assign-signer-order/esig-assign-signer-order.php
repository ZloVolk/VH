...
if(class_exists( 'WP_E_Addon' ))
{
	...
	$esign_addons->esign_update_check('7881','1.4.4');
}   
...
require_once( dirname( __FILE__ ) . '/includes/esig-assign-signer-order.php' );
...
//if (is_admin()) {
	...
	require_once( dirname( __FILE__ ) . '/admin/esig-assign-signer-order-admin.php' );
        add_action( 'wp_esignature_loaded', array( 'ESIG_ASSIGN_ORDER_Admin', 'get_instance' ) );
        require_once( dirname( __FILE__ ) . '/admin/esig-assign-approval-signer-admin.php' );
        add_action( 'wp_esignature_loaded', array( 'ESIG_ASSIGN_APPROVAL_SIGNER_Admin', 'get_instance' ) );
