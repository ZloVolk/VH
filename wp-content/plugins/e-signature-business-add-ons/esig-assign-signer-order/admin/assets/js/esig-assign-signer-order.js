(function ($) {
    ...
    $.fn.signer_order_checked = function () {
        ...
        var fname = $("input[name='recipient_fnames\\[\\]']").map(function () {
            return $(this).val();
        });

        var signer_name = $("input[name='recipient_emails\\[\\]']").map(function () {
            return $(this).val();
        });
    ...
    $.fn.signer_order_unchecked = function () {
        ...
        var fname = $("input[name='recipient_fnames_ajax\\[\\]']").map(function () {
            return $(this).val();
        });

        var signer_name = $("input[name='recipient_emails_ajax\\[\\]']").map(function () {
            return $(this).val();
        });
    ...
    $.fn.signer_order_unchecked_view = function () {
        ...
        for (i = 0; i < fname.length; i++) {
            ...
            html += '<div id="signer_main">' +
                    '<input type="text" name="recipient_fnames[]" placeholder="Signers Name" value="' + fname[i] + '"  />' +
                    '<input type="text" name="recipient_emails[]" placeholder="Signer Email" style="width:213px;"  value="' + signer_name[i] + '"  />' + delicon + '</div>';
