...
if (!class_exists('ESIG_ASSIGN_APPROVAL_SIGNER_Admin')) :
    ...
    class ESIG_ASSIGN_APPROVAL_SIGNER_Admin {
        ...
        private function __construct() {
            ...
            add_action('esig_signature_saved', array($this, 'signature_saved'), 9999, 1);
         ...
         final function esig_sif_value_filter($user_invite, $document_id) {
        ...
        public function document_after_save($args) {
            ...
            $api = new WP_E_Api();
            $api->meta->add($document_id, 'esig_assign_approval_signer', $_POST['esig_assign_approval_signer']);
        }
        ...
        public function assign_approval_signer($advanced_more_options) {
            ...
            $advanced_more_options .='<p><a href="#" class="tooltip">
                                    ...
                                    </a><input type="checkbox" id="esig_assign_approval_signer" name="esig_assign_approval_signer" value="1" ' . $esig_assign_approval_signer_checked . '><span style="margin-left:4px;">' . __('Assign an approval signer to sign and approve this document once a signature is added - <a href="https://www.approveme.me/wp-digital-signature-plugin-docs/article/approval-signer/" class="esig-light-link">help article</a>', 'esig-order') . '</span></p>';
        ...
        public function signature_saved($args) {
            ...
            if ($document_type == "normal") {
                 if(!WP_E_Sig()->meta->get($document_id,'approval_invitation_'.$args['invitation']->invitation_id))
        ...
        public function send_invite_approval_signer($old_doc_id, $document_id, $first_signer_id = false) {
            ...
            for ($i = 1; $i < count($signers[1]); $i++) {
                ...
                WP_E_Sig()->meta->add($document_id, "approval_invitation_" . $invite_id, 1);
            }


            // saving signer order 
            if ($signer_order == "active") {
                $api->setting->set('esig_assign_signer_order' . $document_id, json_encode($assign_signer_order));
        ...
        public function enqueue_admin_scripts() {
            ...
            if (in_array($screen->id, $admin_screens)) {
                wp_enqueue_script($this->plugin_slug . '-popup-script', plugins_url('assets/js/esig-assign-signer-order-popup.js', __FILE__), array('jquery'), ESIG_ASSIGN_ORDER::VERSION, true);
            }
        }
